// ─── Email Service (Brevo) ────────────────────────────────────
// Fully decoupled email service. Add new templates by adding methods.
...
export class EmailService {
  ...
  private async send(options: EmailOptions): Promise<boolean> {
    try {
      ...
      console.log(`[EmailService] Request body:`, JSON.stringify(body, null, 2));
      ...
      console.log(`[EmailService] Brevo response status: ${response.status} ${response.statusText}`);
      if (!response.ok) {
        const errText = await response.text();
        console.error(`[EmailService] Brevo error body:`, errText);
        return false;
      }
      const data = await response.json();
      console.log(`[EmailService] Brevo success body:`, JSON.stringify(data));
      return true;
    } catch (err) {
      console.error(`[EmailService] Exception during send:`, err);
      return false;
    }
  }

  async sendEmailVerification(to: string, firstName: string, token: string, appUrl: string): Promise<boolean> {
    const verifyUrl = `${appUrl}/auth/verificar-email?token=${token}`
    return this.send({
      to,
      subject: 'Verifica tu cuenta en NutriPro',
      html: emailVerificationTemplate(firstName, verifyUrl),
    })
  }

  async sendWelcomeEmail(to: string, firstName: string, appUrl: string): Promise<boolean> {
    return this.send({
      to,
  ...
  async sendPasswordReset(to: string, firstName: string, token: string, appUrl: string): Promise<boolean> {
    const resetUrl = `${appUrl}/auth/nueva-contrasena?token=${token}`
    return this.send({
      to,
      subject: 'Recupera tu contraseña en NutriPro',
      html: passwordResetTemplate(firstName, resetUrl),
    })
  }

  async sendNewQuestionnaire(adminEmail: string, clientName: string, clientEmail: string, appUrl: string): Promise<boolean> {
    return this.send({
      to: adminEmail,
      subject: `Nuevo formulario nutricional — ${clientName}`,
      html: newQuestionnaireTemplate(clientName, clientEmail, appUrl),
    })
  }

  async sendNewAppointmentRequest(adminEmail: string, clientName: string, date: string, time: string): Promise<boolean> {
    return this.send({
      to: adminEmail,
      subject: `Nueva cita solicitada — ${clientName}`,
      html: newAppointmentAdminTemplate(clientName, date, time),
    })
  }

  async sendAppointmentConfirmed(to: string, firstName: string, date: string, time: string, appUrl: string): Promise<boolean> {
    return this.send({
      to,
      subject: 'Tu cita ha sido confirmada — NutriPro',
      html: appointmentConfirmedTemplate(firstName, date, time, appUrl),
    })
  }

  async sendAppointmentCancelled(to: string, firstName: string, date: string, time: string, reason?: string): Promise<boolean> {
    return this.send({
      to,
      subject: 'Tu cita ha sido cancelada — NutriPro',
      html: appointmentCancelledTemplate(firstName, date, time, reason),
    })
  }

  async sendContactForm(adminEmail: string, name: string, email: string, subject: string, message: string): Promise<boolean> {
    return this.send({
      to: adminEmail,
...
const baseStyle = `
  ...
  border-radius: 24px;
  box-shadow: 0 10px 30px rgba(45, 30, 27, 0.04);
  overflow: hidden;
`
...
function emailVerificationTemplate(firstName: string, verifyUrl: string): string {
  return wrapEmail('Verifica tu cuenta', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Hola, ${firstName}</h2>
    <p style="color:#4A453F;margin:0 0 16px;">Gracias por registrarte en NutriPro. Para activar tu cuenta, haz clic en el botón de abajo:</p>
    <a href="${verifyUrl}" style="${btnStyle}">Verificar mi cuenta</a>
    <p style="color:#A39E98;font-size:13px;margin:24px 0 0 0;">Este enlace expira en 24 horas.</p>
  `)
}

function welcomeTemplate(firstName: string, appUrl: string): string {
  return wrapEmail('Bienvenido a NutriPro', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Hola, ${firstName}</h2>
    <p style="color:#4A453F;margin:0 0 16px;">Nos hace mucha ilusión darte la bienvenida a <strong>NutriPro</strong>. Tu cuenta ha sido creada con éxito.</p>
    <p style="color:#4A453F;margin:0 0 16px;">A partir de ahora podrás solicitar citas, acceder a tus dietas personalizadas y rellenar tus formularios nutricionales desde tu panel de cliente.</p>
    <p style="color:#4A453F;margin:0 0 16px;">Si tienes cualquier duda, no dudes en ponerte en contacto con nosotros.</p>
    <a href="${appUrl}/auth/login" style="${btnStyle}">Acceder a mi panel</a>
  `)
}

function passwordResetTemplate(firstName: string, resetUrl: string): string {
  return wrapEmail('Recupera tu contraseña', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Hola, ${firstName}</h2>
    <p style="color:#4A453F;margin:0 0 16px;">Recibimos una solicitud para restablecer tu contraseña. Haz clic en el botón para crear una nueva:</p>
    <a href="${resetUrl}" style="${btnStyle}">Restablecer contraseña</a>
    <p style="color:#A39E98;font-size:13px;margin:24px 0 8px 0;">Nota: Este enlace expira en 1 hora y solo puede usarse una vez.</p>
    <p style="color:#A39E98;font-size:12px;margin:0;">Si no solicitaste este cambio, ignora este correo. Tu contraseña no cambiará.</p>
  `)
}

function newQuestionnaireTemplate(clientName: string, clientEmail: string, appUrl: string): string {
  return wrapEmail('Nuevo formulario nutricional', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Nuevo formulario recibido</h2>
    <p style="color:#4A453F;margin:0 0 16px;"><strong>${clientName}</strong> (${clientEmail}) ha enviado su formulario nutricional.</p>
    <p style="color:#4A453F;margin:0 0 16px;">Accede al panel de administración para revisar las respuestas y asignar un plan personalizado.</p>
    <a href="${appUrl}/admin/formularios" style="${btnStyle}">Ver formulario</a>
  `)
}

function newAppointmentAdminTemplate(clientName: string, date: string, time: string): string {
  return wrapEmail('Nueva cita solicitada', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Nueva solicitud de cita</h2>
    <p style="color:#4A453F;margin:0 0 16px;"><strong>${clientName}</strong> ha solicitado una cita:</p>
    <ul style="color:#4A453F;line-height:2;padding-left:20px;margin:0 0 16px 0;">
      <li><strong>Fecha:</strong> ${date}</li>
      <li><strong>Hora:</strong> ${time}</li>
    </ul>
    <p style="color:#4A453F;margin:0;">Accede al panel para confirmar o reprogramar.</p>
  `)
}

function appointmentConfirmedTemplate(firstName: string, date: string, time: string, appUrl: string): string {
  return wrapEmail('Cita confirmada', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Tu cita está confirmada</h2>
    <p style="color:#4A453F;margin:0 0 16px;">Hola <strong>${firstName}</strong>, tu cita ha sido confirmada:</p>
    <div style="background:#FAF0F4;border:1px solid #EAD6E2;border-radius:16px;padding:20px;margin:20px 0;">
      <p style="margin:0 0 8px 0;color:#6B2245;font-weight:600;"><strong>Fecha:</strong> ${date}</p>
      <p style="margin:0;color:#6B2245;font-weight:600;"><strong>Hora:</strong> ${time}</p>
    </div>
    <a href="${appUrl}/dashboard/citas" style="${btnStyle}">Ver mis citas</a>
  `)
}

function appointmentCancelledTemplate(firstName: string, date: string, time: string, reason?: string): string {
  return wrapEmail('Cita cancelada', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Tu cita ha sido cancelada</h2>
    <p style="color:#4A453F;margin:0 0 16px;">Hola <strong>${firstName}</strong>, lamentamos informarte que tu cita ha sido cancelada:</p>
    <div style="background:#FAF0F0;border:1px solid #EAD6D6;border-radius:16px;padding:20px;margin:20px 0;">
      <p style="margin:0 0 8px 0;color:#C24141;font-weight:600;"><strong>Fecha:</strong> ${date}</p>
      <p style="margin:0 0 8px 0;color:#C24141;font-weight:600;"><strong>Hora:</strong> ${time}</p>
      ${reason ? `<p style="margin:0;color:#C24141;font-weight:600;"><strong>Motivo:</strong> ${reason}</p>` : ''}
    </div>
    <p style="color:#4A453F;margin:0;">Puedes solicitar una nueva cita desde tu panel.</p>
  `)
}

function contactFormTemplate(name: string, email: string, subject: string, message: string): string {
  return wrapEmail('Mensaje de contacto', `
    <h2 style="color:#2D1E1B;margin:0 0 16px;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Nuevo mensaje de contacto</h2>
    <ul style="color:#4A453F;line-height:2;padding-left:20px;margin:0 0 16px 0;">
      <li><strong>Nombre:</strong> ${name}</li>
      <li><strong>Email:</strong> ${email}</li>
      <li><strong>Asunto:</strong> ${subject}</li>
    </ul>
    <div style="background:#FAF9F7;border:1px solid #EAE5E0;border-radius:16px;padding:20px;margin:20px 0;">
      <p style="color:#4A453F;line-height:1.7;margin:0;">${message.replace(/\n/g, '<br>')}</p>
    </div>
    <a href="mailto:${email}" style="${btnStyle}">Responder</a>
  `)
}
